    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: ElevatedButton(
          onPressed: () {
            PrinterService()
                .printSalesReport(context, controller.salesReportList);
          },
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                _buildStatCard(
                    "Net Sales",
                    _formatCurrency(report.netSales ?? "0"),
                    Icons.attach_money,
                    Colors.green),
                _buildStatCard(
                    "Avg. Sales",
                    _formatCurrency(report.averageSales ?? "0"),
                    Icons.trending_up,
                    Colors.blue),
                _buildStatCard("Orders", report.totalOrders?.toString() ?? "0",
                    Icons.shopping_cart, Colors.orange),
                _buildStatCard("Items", report.totalItems?.toString() ?? "0",
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _buildStatCard("Tax", _formatCurrency(report.totalTax ?? "0"),
                    Icons.receipt, Colors.red),
                _buildStatCard(
                    "Shipping",
                    _formatCurrency(report.totalShipping ?? "0"),
                    Icons.local_shipping,
                    Colors.brown),
                _buildStatCard(
                    "Discount",
                    _formatCurrency(report.totalDiscount ?? "0"),
                    Icons.discount,
                    Colors.pink),
                _buildStatCard(
                    "Customers",
                    report.totalCustomers?.toString() ?? "0",
                    Icons.people,
                    Colors.teal),
              ],
            ),
